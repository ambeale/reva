...
class FavoriteClicker extends React.Component {
  ...
  componentDidMount() {
    if (userId !== "None") {
      fetch(`/is-favorite?restaurant=${restaurantId}&user_id=${userId}`).then(response => response.json()).then(data => {
        if (data === true) {
  ...
  addFavorite() {
    if (userId !== "None") {
      $.post("/update-favorite", { "restaurant_id": restaurantId }, this.favSuccess);
    } else {
      this.notLoggedIn();
    }
  }
  ...
  render() {
    return React.createElement("span", { className: this.state.value, onClick: this.addFavorite });
  }
}
...
ReactDOM.render(React.createElement(FavoriteClicker, null), document.getElementById('fav'));
